...
function createPost(content){
  const post = document.createElement('div'); //<div> 요소 생성
  post.classList.add('post');  //post 클래스 추가
...
function showPage(page){
  ...
  const endIdx = Math.min(startIdx + pageSize, totalItems); //현재페이지 마지막 인덱스 계산
  //시작인덱스부터 마지막 인덱스까지 반복하면서 게시물 생성 및 추가
  for(let i = startIdx; i < endIdx; i++){
    const post = createPost(`게시물 ${i+1}`); //게시물 생성
    contentContainer.appendChild(post); //게시물 컨테이너에 추가
  }
}
...
for(let i = 1; i <= totalPages; i++){
  ...
  if(i === 1){
    li.classList.add('active'); //첫 페이지에 active 클래스를 추가
  }

  li.addEventListener("click", onPageClick); // 페이지 번호 클릭 이벤트 핸들러 추가
  
  pagination.appendChild(li); //페이지 번호 <ul>요소에 추가
}

paginationConatiner.appendChild(pagination); //페이지 번호<ul>요소를 페이지 네비게이션 컨테이너 추가
